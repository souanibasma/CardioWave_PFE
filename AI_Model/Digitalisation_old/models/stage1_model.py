...
def F_binary_cross_entropy_with_logits(logit, truth, pos_weight=None):
	#support for ignore pixel

	mask = truth >= 0 #negative pixel are ignored
	if mask.sum() == 0:
...
class Net(nn.Module):
	def __init__(self, pretrained=True, cfg=None):
		...
		self.gridpoint  = nn.Conv2d(decoder_dim[-1], 1, kernel_size=1) #point, sigmoid
		self.gridhline  = nn.Conv2d(decoder_dim[-1], 44+1, kernel_size=1) #hline, vline
		self.gridvline  = nn.Conv2d(decoder_dim[-1], 57+1, kernel_size=1) #hline, vline
		self.marker = nn.Conv2d(decoder_dim[-1], 13+1, kernel_size=1)  #softmax
	...
	def forward(self, batch):
		...
		if 'loss' in self.output_type:
			...
			output['gridpoint_loss'] = F_binary_cross_entropy_with_logits(
				gridpoint, batch['gridpoint'].to(device),
				pos_weight=torch.tensor([10]).to(device)#.reshape(3,1,1)
			)
			output['gridhline_loss'] = F_cross_entropy(
				gridhline, batch['gridhline'].to(device))
			output['gridvline_loss'] = F_cross_entropy(
				gridvline, batch['gridvline'].to(device))
			output['grid_loss'] = 2*output['gridpoint_loss']+output['gridhline_loss']+output['gridvline_loss']
...
def run_check_net():
	...
	batch = {
		...
		'gridpoint': torch.from_numpy(np.random.uniform(0,1, (batch_size, 1, H, W))).float(),
		'gridhline': torch.from_numpy(np.random.choice(45, (batch_size, H, W))).byte(),
		'gridvline': torch.from_numpy(np.random.choice(58, (batch_size, H, W))).byte(),
		'marker': torch.from_numpy(np.random.choice(14, (batch_size, H, W))).byte(),
	}
