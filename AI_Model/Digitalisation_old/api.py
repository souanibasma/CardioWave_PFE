...
# Forcer le GPU ou utiliser CPU
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"INFO DEVICE INITIALISATION: {DEVICE} (CUDA available: {torch.cuda.is_available()})")
...
def generate_full_lead_ii_plot(signal_ii, title, output_path):
    ...
    t = np.arange(len(signal_ii))
    ax.set_title(title, fontsize=16, fontweight="bold")
    ax.plot(t, signal_ii, linewidth=0.8, color="blue", label="Lead II")
    ax.grid(True, alpha=0.3)
...
app = FastAPI(title="ECG Digitization API")
...
def digitize_ecg(
    ...
    try:
        ...
        # Pipeline
        image_norm, _, _ = run_stage0(image)
        if image_norm is None: raise RuntimeError("Erreur Stage 0")
        
        image_rect, _ = run_stage1(image_norm)
        if image_rect is None: raise RuntimeError("Erreur Stage 1")

        series_4 = run_stage2(image_rect, sample_length=sample_length)
        if series_4 is None: raise RuntimeError("Erreur Stage 2")
        ...
        np.save(output_file_path, series_4)
        ...
        print(f"============================================================")
        print(f"DIGITIZATION TOOK: {elapsed_time:.2f} seconds ({elapsed_time/60:.2f} minutes)")
        print(f"============================================================")
        ...
        return ProcessResponse(
            ...
            series_shape=list(series_4.shape),
            npy_file=f"{BASE_URL}/files/digitalised/{filename}_extracted_12leads.npy",
            normalised_image=f"{BASE_URL}/files/normalised/{filename}_normalised.png",
            rectified_image=f"{BASE_URL}/files/rectified/{filename}_rectified.png",
            plot_4leads=f"{BASE_URL}/files/digitalised/{filename}_plot_4leads.png",
            plot_12leads=f"{BASE_URL}/files/digitalised/{filename}_plot_12leads.png",
            plot_full_lead_ii=f"{BASE_URL}/files/digitalised/{filename}_plot_full_lead_ii.png",
        )
